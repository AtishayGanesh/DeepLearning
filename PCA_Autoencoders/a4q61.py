...
class AE(nn.Module):
    def __init__(self):
        ...
        self.conv5 = nn.ConvTranspose2d(in_channels=16, out_channels=8,kernel_size= 3, stride = 2,padding=0)
        self.conv6 = nn.ConvTranspose2d(in_channels=8,out_channels =1,kernel_size= 2, stride = 2,padding=1)
    ...
    def test_epoch(self,type_set):
        ...
        with torch.no_grad():
            for data, target in curr_loader:
                ...
                plt.imsave(f'.\\Progress61\\rig.jpeg',data2[0,:,:])
                plt.imsave(f'.\\Progress61\\back.jpeg',output2[0,:,:])
                # plt.imsave(f'.\\Progress61\\orig_r.jpeg',data2r[0,:,:])
                # plt.imsave(f'.\\Progress61\\back_r.jpeg',output2r[0,:,:])
                s = (self.model.conv5.weight.data)
                print(s.shape)
                for i in range(5):
                    plt.imsave(f'.\\Progress61\\w{i}.png',s.cpu().detach().numpy()[i,i,:,:])             
