...
def hook(module, input, output):
    ...
    print(output[0])
    print('Average Value of activations',torch.pow(torch.sum(torch.abs(output[0]))/list((output[0].shape))[0],0.5   ))

class Neural_Network():
    def __init__(self,train_loader,valid_loader,test_loader,device,size,hidden_dim,sparse):
    ...
    def train_epoch(self,epoch):
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            if self.sparse:
                l1_norm = torch.norm(self.model.fc4.weight, p=1)
                loss += l1_norm*2
            loss.backward()
            self.optimizer.step()
    ...
    def test_epoch(self,type_set):
        ...
        if type_set== 'valid':
            ...
            self.writer.add_scalar('Loss/valid', loss,self.steps)
        ...
        with torch.no_grad():
            for data, target in curr_loader:
                ...
                plt.imsave(f'.\\Progress3\\{self.hidden_dim}{self.sparse}orig.jpeg',data2[0,:,:])
                plt.imsave(f'.\\Progress3\\{self.hidden_dim}{self.sparse}back.jpeg',output2[0,:,:])
                plt.imsave(f'.\\Progress3\\{self.hidden_dim}{self.sparse}orig_r.jpeg',data2r[0,:,:])
                plt.imsave(f'.\\Progress3\\{self.hidden_dim}{self.sparse}back_r.jpeg',output2r[0,:,:])
                for i in range(5):
                    plt.imsave(f'.\\Progress3\\{self.hidden_dim}{self.sparse}{i}w.jpeg',self.model.fc4.weight.data.cpu().detach().numpy()[i,:].reshape(28,28) )

                print(F.mse_loss(output.squeeze(), data.squeeze(), reduction='sum').item()/784)
                if type_set != 'valid':
                    self.model.fc4.register_forward_hook(hook)
                    output = self.model(data)
...
if __name__=='__main__':
    ...
    c = Neural_Network(train_loader,valid_loader,test_loader,device,size,args.dim,args.sparse)
    c.train()
