...
class AE(nn.Module):
    def __init__(self):
        ...
        self.conv6 = nn.Conv2d(in_channels=8,out_channels =1,kernel_size= 3, stride = 1,padding=1)
    ...
    def forward(self, x):
        ...
        x = F.max_unpool2d(enc,i3,kernel_size=2,output_size=(batch_size,16,7,7))
        x = F.relu(self.conv4(x))
        x = F.max_unpool2d(x,i2,kernel_size=2,output_size=(batch_size,16,14,14))
        x = F.relu(self.conv5(x))
        x = F.max_unpool2d(x,i1,kernel_size=2,output_size=(batch_size,8,28,28))
    ...
    def test_epoch(self,type_set):
        ...
        with torch.no_grad():
            for data, target in curr_loader:
                ...
                plt.imsave(f'.\\Progress60\\rig.jpeg',data2[0,:,:])
                plt.imsave(f'.\\Progress60\\back.jpeg',output2[0,:,:])
                plt.imsave(f'.\\Progress60\\orig_r.jpeg',data2r[0,:,:])
                plt.imsave(f'.\\Progress60\\back_r.jpeg',output2r[0,:,:])
                s = (self.model.conv5.weight.data)
                print(s.shape)
                for i in range(5):
                    plt.imsave(f'.\\Progress60\\w{i}.png',s.cpu().detach().numpy()[i,i,:,:])             
